...
resource "google_compute_instance" "api" {
  ...
  metadata_startup_script = "echo '${data.google_secret_manager_secret_version.secret-version.secret_data}' | sudo tee /.env && docker run --env-file=/.env -p 3000:3000 gcr.io/${var.project}/opentourney-api:latest"
